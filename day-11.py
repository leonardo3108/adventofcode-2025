...
Solution:
  - Parse input into a graph (dict of lists)
  - Use DFS to count all paths from 'you' to 'out'
...
Solution:
 - Change the start node to 'svr'
 - Cache counted paths to avoid recomputation
 - For part two, break the path counting into segments:
    - From 'svr' to 'fft'
    - From 'fft' to 'dac'
    - From 'dac' to 'out'
    - And the other order:
    ...
    - From 'dac' to 'fft'
    - From 'fft' to 'out'
 - Multiply the path counts for each segment and sum the two orders
...
input = open('input-11.txt').read()

flow = {}
for line in input.strip().split('\n'):
    parts = line.split(': ')
    flow[parts[0]] = parts[1].split(' ')
    
# print(flow)

# DFS to count all paths
def count_paths(flow, start, end):
    if start not in counted_paths:
        if start == end:
            counted_paths[start] = 1
        else:
            path_count = 0
            for neighbor in flow[start]:
                path_count += count_paths(flow, neighbor, end)
            counted_paths[start] = path_count
    return counted_paths[start]

# Part One
counted_paths = {}
part_one_paths = count_paths(flow, 'you', 'out')
print("Part One - Total paths from 'you' to 'out':", part_one_paths)

# Part Two
counted_paths = {'out': 0}
part_two_path1 = count_paths(flow, 'svr', 'fft')
counted_paths = {'out': 0}
part_two_path1 *= count_paths(flow, 'fft', 'dac')
counted_paths = {}
part_two_path1 *= count_paths(flow, 'dac', 'out')

counted_paths = {'out': 0}
part_two_path2 = count_paths(flow, 'svr', 'dac')
counted_paths = {'out': 0}
part_two_path2 *= count_paths(flow, 'dac', 'fft')
counted_paths = {}
part_two_path2 *= count_paths(flow, 'fft', 'out')

print(f"Part Two - Paths from 'svr' to 'out' visiting 'fft' and 'dac': {part_two_path1 + part_two_path2}")
